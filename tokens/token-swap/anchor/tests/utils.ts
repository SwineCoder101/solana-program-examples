...
export const expectAnchorError = async (promise: Promise<any>, code: string) => {
    let error: unknown;
    try {
        await promise;
    } catch (err) {
    ...
    if (error === undefined) {
        throw new Error(`Expected transaction to fail with ${code}`);
    }
    if (!(error instanceof anchor.AnchorError)) {
        throw new Error(`Expected AnchorError ${code}, got: ${String(error)}`);
    }
    expect(error.error.errorCode.code).to.equal(code);
};
...
export const mintingTokens = async ({
    ...
    await mintToHolder({ connection, creator, holder, mintAKeypair, mintBKeypair, mintedAmount, decimals });
};

export const mintToHolder = async ({
    connection,
    creator,
    holder,
    mintAKeypair,
...
export function createValues(defaults?: TestValuesDefaults): TestValues {
    ...
    // Making sure tokens are in the right order (unless explicit mints are given)
    const mintAKeypair = defaults?.mintAKeypair || Keypair.generate();
    let mintBKeypair = defaults?.mintBKeypair || Keypair.generate();
    while (
        !defaults?.mintBKeypair &&
        new anchor.BN(mintBKeypair.publicKey.toBytes()).lt(new anchor.BN(mintAKeypair.publicKey.toBytes()))
    ) {
